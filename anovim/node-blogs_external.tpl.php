...
// Author (posted by)
$original_site_title = $node->links['feedapi_feed']['title'];
$original_site_title = str_replace('Feed: ','',$original_site_title);
$original_article_url = $node->links['feedapi_original']['href'];
...
if ($page == 0) {
  ...
  <h4 class="author">
    Author: <a href="<?php print $original_article_url ?>" class="nounderline"><?php print $original_site_title ?></a>
  </h4>
  <?php
}
...
if ($submitted) {
  ...
  ?>
  <h4 class="submitted">
    <?php if ($page == 0) { print t('Posted by: ') . $original_site_title .' | '; } ?>
...
?>
  ...
  <div class="content clear-block">
    ...
    if ($page == 0) {
      ...
      $text = str_replace("\n","</p>\n<p>",$text);  // add paragraphs
      $text = str_replace('<br />','',$text);       // delete line breaks
    }
    print '<p>'.$text.'</p>';
    
    if ($page != 0) {
      print l('Back to '.$original_site_title ,'feed-item/'.$node->feedapi_node->feed_nids[key($node->feedapi_node->feed_nids)]);
    }
    ?>
